...
#include "RFT_IF_PACKET_Rev1.3.h" // RFT F/T SENSOR DATA FIELD & UART PACKET HANDLING SOURCE
...
public:
	...
	BOOL	openPort(float forceDivider, float torqueDivider, BYTE bPort = 1, DWORD rate = 19200, BYTE byteSize = 8, BYTE stop = ONESTOPBIT, BYTE parity = NOPARITY, BYTE bFlowCtrl = FC_NONE, bool callback_enable = false);
	BOOL	closePort(void);
...
#ifdef NEW_FILTER
	bool set_FT_Filter_Type(int filter_type, int sub_type, int MAcount);		// set filter
#else
