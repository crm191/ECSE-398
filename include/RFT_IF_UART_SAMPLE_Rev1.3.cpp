...
// for only 485 communication 
bool fSetCommandsgSendReady = false;
unsigned char m_SetCommandMsgBuff[UART_COMMAND_PACKET_SIZE];
...
BOOL CRT_RFT_UART::openPort(float forceDivider, float torqueDivider, BYTE bPort, DWORD rate, BYTE byteSize, BYTE stop, BYTE parity, BYTE bFlowCtrl, bool callback_enable)
{
...
void CRT_RFT_UART::readWorker(void)        // read worker.
{
	...
	while (m_bConnected)
	{
		...
		if ((dwEvtMask & EV_RXCHAR) == EV_RXCHAR)
		{
			...
			// 상위 제어기에서 멈춤 명령을 전송하지 않고 PC 소프트웨어가 멈춘 경우 데이터 전송 정지 명령을 보낸다.
			...
			if (fSetCommandsgSendReady)
			{
				DWORD written;
				(bool)write(m_SetCommandMsgBuff, UART_COMMAND_PACKET_SIZE, &written);
				fSetCommandsgSendReady = false;
			}
...
bool CRT_RFT_UART::rqst_FT_Stop(void)
{
	...
	DWORD written;
	if ((m_nCurrMode == CMD_FT_CONT))
	{
		memcpy(m_SetCommandMsgBuff, m_txBuff, UART_COMMAND_PACKET_SIZE);
		fSetCommandsgSendReady = true;
	}
	else
	{
		result = (bool)write(m_txBuff, txPktSize, &written);
	}
	//CONSOLE_S("written: %d\n", written);
	return result;
}
...
#ifdef NEW_FILTER
bool CRT_RFT_UART::set_FT_Filter_Type(int filter_type, int sub_type, int MAcount)
#else
...
#endif
{
	...
	int txPktSize = UART_COMMAND_PACKET_SIZE;
#ifdef NEW_FILTER
	m_RFT_IF_PACKET.UPG_set_filter_type(m_txBuff, filter_type, sub_type, MAcount);
#else
...
void CRT_RFT_UART::rcvd_FT(void)
{
	...
	if (m_nRcvdBufferIdx >= pkt_size)
	{
		...
		strPCTime.Format("%02d:%02d:%02d.%03ld",
			cur_time.wHour,
			cur_time.wMinute,
			cur_time.wSecond,
		...
		if (m_RFT_IF_PACKET.UART_packet_processing(m_rcvdBuff, m_nCurrMode))
		{
			...
			{
				m_nlogDataCount++;
				fprintf(m_pLogFile, strPCTime + " %f %f %f %f %f %f\n",
...
bool CRT_RFT_UART::startLogging(CString logfileName)
{
	bool result = true;
...
bool CRT_RFT_UART::stopLogging(void)
{
	m_logfile_mutex.lock();
	if (m_pLogFile != NULL)
	{
