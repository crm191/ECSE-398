...
// Rev1.21 2023.02.08 krhan 
// 온도보상 정밀도를 높이기위해 온도값을 1byte 에서 2byte 로나누어 전송. 
// 기존 FW와 호환을 위해 정수부(1byte) + (소수점 아래첫번째수*10)(1byte) 로 나누어 전송함
// 이동평균필터 수량의 설정 및 읽기 항목  관련 부분 수정함

#include "RFT_IF_PACKET_Rev1.3.h"
...
#ifdef NEW_FILTER	
	m_rcvd_filter_setting_value_1 = 0; // krhan for LPF 필터
	m_rcvd_filter_setting_value_2 = 0; // krhan for 이동평균필터
#else
...
// krhan 이동평균필터 인수 항목 추가
bool CRT_RFT_IF_PACKET::DFG_set_filter_type(unsigned char data_field[COMMAND_PACKET_DATA_FIELD_SIZE], unsigned char filter_type,
#ifdef NEW_FILTER	
	unsigned char set_value_1, unsigned char set_value_2)
#else
...
#ifdef NEW_FILTER	
	data_field[2] = set_value_1; // krhan
	data_field[3] = set_value_2; // krhan
#else
...
void CRT_RFT_IF_PACKET::convertPacket_To_Force(unsigned char* rcvdPacket, float* force, float forceDivider, float torqueDivider)
{
	...
	unsigned short temp;
#ifdef NEW_FILTER	
	int integer_temperature, fraction_temperature; //krhan
#endif
	...
	// 온도분해능을 소수점 첫째자리까지 높힘
#ifdef NEW_FILTER	
	integer_temperature = (int)(rcvdPacket[13] < 125 ? rcvdPacket[13] : -(256 - rcvdPacket[13])); // krhan
	fraction_temperature = (int)(rcvdPacket[14] < 125 ? rcvdPacket[14] : -(256 - rcvdPacket[14])); // krhan
	m_rcvdTemperature = integer_temperature + (float)(fraction_temperature / 10.0);//krhan

#else
	m_rcvdTemperature = (double)(rcvdPacket[13] < 125 ? rcvdPacket[13] : -(256 - rcvdPacket[13])); // º¯È¯	
#endif
}
...
bool CRT_RFT_IF_PACKET::rcvd_data_field_processing(unsigned char data_field[RESPONSE_PACKET_DATA_FIELD_SIZE], unsigned char check_command_type)
{
...
#ifdef NEW_FILTER		
		m_rcvd_filter_setting_value_1 = data_field[2]; // krhan
		m_rcvd_filter_setting_value_2 = data_field[3]; // krhan
#else
	...
	case CMD_FT_CONT:
		...
		//kjw		m_measure_error = data_field[15];
		break;
...
//krhan 이동평균필터 인수 설정을 위해 수정함
bool CRT_RFT_IF_PACKET::UPG_set_filter_type(unsigned char packet_buff[UART_COMMAND_PACKET_SIZE], unsigned char filter_type,
#ifdef NEW_FILTER
	unsigned char set_value_1, unsigned char set_value_2)
#else
unsigned char set_value)
...

{
	...
	packet_buff[0] = SOP;
#ifdef NEW_FILTER
	DFG_set_filter_type(packet_buff + 1, filter_type, set_value_1, set_value_2); // krhan
#else
